...
//	protected static int MAX_DATA_TO_READ_FROM_BUFFER = 15;
	protected static int MAX_DATA_TO_READ_FROM_BUFFER = 16;
	/**
	...
	protected void uploadNextDataFromFifoToRemoteDevice() {
		...
		if (mTxDest.length() >= 0 && mTxBuffer.getSize() > 0) {
	...
	{
		Log.i(TAG, "writeToFifoAndUploadDataToRemoteDevice: data = " + data);
		Log.i(TAG, "writeToFifoAndUploadDataToRemoteDevice: datalength = " + data.length());
		mTxBuffer.write(data);
		uploadNextDataFromFifoToRemoteDevice();
	};

	public static byte[] HexCommands2DecimalCommands(String hc)
	{
		StringTokenizer st1 =  new StringTokenizer(hc, " ");
		int count = st1.countTokens();
		byte[] decimalVal = new byte[count];
		int i= 0;
		while (st1.hasMoreTokens())
		{
			decimalVal[i++] = (byte) Integer.parseInt(st1.nextToken(), 16);          //       16 used to indicate hexadecimal
		}
		return decimalVal;
	}
...
/*
			String auscStartCmd = "3B 0A 11 00 03 00 04 02 00 5F";
			String auscStopCmd = "3B 08 13 00 01 00 00 57";
			String syncCmd = "3B 08 10 00 01 00 00 54";
			String recordCmd = "3B 08 14 00 01 00 00 58";
			String getRawFileCmd = "3B 08 18 01 01 00 00 5D";
*/


			//byte[] cmd = HexCommands2DecimalCommands(getRawFileCmd);
			mCharRx.setValue(dataToBeSend.getBytes());
			...
			BluetoothGattCharacteristic ch)
	{
		...
		Log.i(TAG, "onCharacteristicChanged: before data read successfully");
		if (VSP_SERVICE.equals(serviceUUID))
		{
